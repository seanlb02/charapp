...
const unfollowUser = async function () {
    ...
    const body = {followee_id : `${userdata.id}`}
    var token = (JSON.parse(localStorage.getItem("tokenKey").replaceAll("", '')))
    const res = await fetch(`http://127.0.0.1:8000/connections/unfollow/`, {
    method: 'DELETE',
    headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    .then(() => setFollowingText(styles.HiddenFollowingText))
    .then(() => setFollowButton(styles.followButton))
} 
...
if(loggedinUser != undefined){
  ...
  return (
    ...
    <section className={styles.treePreview}>
      <div className={styles.treeText}>[Not sure what will go here, maybe 'highlighted' branches?]</div>
    </section>
    </div>
  )
}
